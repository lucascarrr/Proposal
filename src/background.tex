...
\begin{definition}
    ...
    A \emph{formal context} is a triple, $\FC$, where $G$ is a finite set of objects, $M$ is a finite set of attributes, and $I\subseteq G\times M$ is an incidence relation such that for an object $g$ and attribute $m$, $(g,m) \in I$ is interpreted as  $g$ \textit{having} $m$, and $(g,m) \not \in I$ as $g$ having $\neg m$.
\end{definition}
%
